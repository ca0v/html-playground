...
export function run() {
  ...
  <input class="filter" placeholder="filter icons"/>
    ...
    keys(markers).forEach(marker => {
        let b = asDom(`<button id="${marker}" class="marker"><svg viewBox="-18 -18 36 36"><path d="${markers[marker]}"></path></svg></button>`);
    ...
    keys(icons).forEach(marker => {
        let b = asDom(`<button id="${marker}" class="icon"><svg viewBox="0 0 36 36"><path d="${icons[marker]}"></path></svg></button>`);
        toolbar.appendChild(b);
        b.addEventListener("click", () => {
            insertIntoEditor(editor, b.querySelector("path"));
        });
        ...
        const doit = () => {
            ...
            if (symbols) {
                symbols.forEach(symbol => {
                    ...
                    let b = asDom(`<button class="icon" title=#${symbol.id}><svg viewBox="${x} ${y} ${width} ${height}"><g>${symbol.innerHTML}</g></svg></button>`);
                    toolbar.appendChild(b);
                    b.addEventListener("click", () => {
